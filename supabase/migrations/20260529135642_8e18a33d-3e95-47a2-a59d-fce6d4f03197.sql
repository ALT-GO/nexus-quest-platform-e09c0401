...
DROP FUNCTION IF EXISTS public.create_public_ticket(text, text, text, text, text, text, text);
...
CREATE OR REPLACE FUNCTION public.create_public_ticket(
  ...
  p_sla_deadline_override timestamptz DEFAULT NULL
)
...
BEGIN
  ...
  v_sla_deadline := COALESCE(p_sla_deadline_override, now() + (v_sla_hours || ' hours')::interval);

  INSERT INTO public.tickets (
    title, category, description, requester, email, department,
    priority, status_id, sla_hours, sla_deadline, ticket_number, parent_ticket_id
  ) VALUES (
    COALESCE(NULLIF(p_title, ''), p_category),
    p_category, p_description, p_requester, p_email, NULLIF(p_department, ''),
    COALESCE(p_priority, 'medium'), v_status_id, v_sla_hours, v_sla_deadline, '', p_parent_ticket_id
  )
...
GRANT EXECUTE ON FUNCTION public.create_public_ticket(text, text, text, text, text, text, text, uuid, timestamptz) TO anon, authenticated;
